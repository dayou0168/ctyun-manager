...
async function ensureLogin(account, target=urls.recharge) {
  ...
  const accountForm=await firstVisible(page.locator("form.account"));const agreementLabel=accountForm?.locator("label.el-checkbox");if(agreementLabel&&await agreementLabel.count()&&!String(await agreementLabel.getAttribute("class")).includes("is-checked")){await accountForm.locator(".el-checkbox__inner").click();await page.waitForTimeout(300);if(!String(await agreementLabel.getAttribute("class")).includes("is-checked"))return {status:"manual_required",message:"无法勾选天翼云用户协议",session};}
