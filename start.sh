...
ENABLE_BROWSER_VNC="${CTYUN_BROWSER_VNC_ENABLED:-${CTYUN_ENABLE_BROWSER_VNC:-0}}"
START_XVFB="${CTYUN_START_XVFB:-$ENABLE_BROWSER_VNC}"
...
if [ "$START_XVFB" != "0" ]; then
  ...
  Xvfb "$DISPLAY_VALUE" -screen 0 1440x900x24 -nolisten tcp >/tmp/xvfb.log 2>&1 &
  sleep 1
  fluxbox -display "$DISPLAY_VALUE" >/tmp/fluxbox.log 2>&1 &
  if [ "$ENABLE_BROWSER_VNC" != "0" ]; then
    sleep 1
    x11vnc -display "$DISPLAY_VALUE" -forever -shared -listen 127.0.0.1 -rfbport 5900 -nopw -noxdamage -o /tmp/x11vnc.log -bg
