...
- 后台默认每 30 分钟同步全部已启用账号；可通过 `CTYUN_BACKGROUND_SYNC_ENABLED` 和 `CTYUN_BACKGROUND_SYNC_SECONDS` 调整。
- 同步按“账号 + 资源类型 + 资源池”独立提交。只有资源池明确同步成功时，平台才会替换该资源池的数据；超时、无权限或接口错误不会误删旧记录。
- 概览数量只统计最近一次成功同步确认的资源。未确认的旧记录仍可在资源页查看，并标记为“缓存已过期”。
- 平台内提交资源操作后，会把官方状态确认任务持久化到 SQLite；即使浏览器关闭或服务重启，任务也会继续执行。
